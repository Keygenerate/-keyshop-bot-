...
# ==================== CONFIGURATION ====================
print("="*60)
print("🔧 KeyShop Bot v4.5 - Starting up...")
print("="*60)
...
if not ADMIN_USER_IDS_STR:
    print("❌ FATAL: ADMIN_USER_IDS environment variable not set!")
    import sys
    sys.exit(1)
...
try:
    ...
    print(f"❌ FATAL: Invalid ADMIN_USER_IDS format: {ADMIN_USER_IDS_STR}")
    import sys
    sys.exit(1)
...
UPI_NAME = os.environ.get("UPI_NAME", "KeyShop Payments")
...
print(f"✅ FORCE_CHANNELS: {FORCE_JOIN_CHANNELS}")
print(f"✅ API_URL: {FIXED_API_URL or 'NOT SET'}")
print(f"✅ UPI_ID: {UPI_ID or 'NOT SET'}")
print("="*60)
...
def home():
    return """
    <html>
    <head><title>KeyShop Bot</title></head>
    <body style="text-align:center; font-family:Arial; padding:50px;">
        <h1>✅ KeyShop Bot is Running!</h1>
        <p>Bot is live and processing orders.</p>
...
def run_web_server():
    """Run Flask web server in background thread for UptimeRobot pings"""
    try:
...
def main():
    ...
    # Start web server in background thread (for UptimeRobot pings)
    web_thread = threading.Thread(target=run_web_server, daemon=True)
    web_thread.start()
    print("🌐 Web server started for UptimeRobot health checks!")
    ...
    # Manual Add Product conversation
    ...
    print('KeyShop Bot v4.4 - SUB-ADMIN SUPPORT')
    print('API Endpoint: ' + FIXED_API_URL)
    print('Duration Mapping:')
    for k, v in DURATION_MAP.items():
        print('   ' + k + ' -> ' + v)
    print('Auto Clear: Every 1 hour')
    print('Sub-Admin Support: ENABLED')
    ...
    if job_queue:
        ...
        print('Auto-clear background task started')
    else:
        print('WARNING: Job queue not available, auto-clear will not run automatically')

    # FIX: Use close_loop=False to avoid __slots__/__dict__ conflict in Python 3.13
    # This is a known issue with python-telegram-bot v20 on Render/Python 3.13
