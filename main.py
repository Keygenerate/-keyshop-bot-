...
def init_db():
    ...
    # Enable WAL mode for better concurrent access (Render pe multiple users ke liye)
...
async def process_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Handle case where user sends a command during screenshot upload
...
async def cancel_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # FIX: Handle both text messages and photo messages (QR code display)
    cancel_text = '❌ Order cancelled.\n\nStart again anytime!'
    cancel_kb = InlineKeyboardMarkup([[InlineKeyboardButton('🏠 Main Menu', callback_data='back')]])

    try:
        # Try editing text first (for non-photo messages)
        await query.edit_message_text(cancel_text, reply_markup=cancel_kb)
    except Exception as e:
        logger.info(f"edit_message_text failed (likely photo msg), trying edit_message_caption: {e}")
        try:
            # For photo messages (QR code display), edit caption instead
            await query.edit_message_caption(caption=cancel_text, reply_markup=cancel_kb)
        except Exception as e2:
