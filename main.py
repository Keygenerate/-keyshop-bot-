#!/usr/bin/env python3
"\"\"\nKeyShop Telegram Bot v4.5 - WEBHOOK RENDER FIX\nFixed:\n- Removed conflicting HTTP server (port conflict fix)\n- Uses Render's PORT env variable properly\n- python-telegram-bot[webhooks] compatible\n\"\""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        MemoryLogHandler()  # Memory mein bhi store karega for /debug
    ]
)
...
class PaymentVerifier:
    def generate_upi_url(self, order_id, amount):
        ...
        tn = f"Order {order_id}"
        tr = order_id
        upi_url = f"upi://pay?pa={pa}&pn={pn}&am={am}&cu={cu}&tn={tn}&tr={tr}"
        return upi_url
    ...
    def generate_payment_msg(self, order_id, amount, product_name, duration):
        """Generate the payment details message (text only, no UPI ID)"""
        lines = [
            "PAYMENT GATEWAY",
            "",
            ...
            "",
            "Scan QR & Pay",
            "Wait 10s after payment then click Check Status."
        ]
        return "\n".join(lines)

    def generate_msg(self, order_id, amount, product_name, duration):
        """Old method - kept for compatibility"""
        return self.generate_payment_msg(order_id, amount, product_name, duration)
...
async def verify_and_deliver(update: Update, context: ContextTypes.DEFAULT_TYPE, order_id: str):
    ...
    if is_success:
        ...
        if key and len(key) > 5:
            ...
            if status_msg:
                await status_msg.edit_text(f"Key format unknown!\n\nID: `{order_id}`\n\nRaw Response:\n```{raw[:400]}```", parse_mode=ParseMode.MARKDOWN)
...
async def product_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for d in durations:
        price = d['price']
        dur = d['duration']
        kb.append([InlineKeyboardButton(f'{dur} - Rs {price:.2f}', callback_data=f'dur_{dur}')])
...
async def buy_product(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Show payment details with Pay via QR button
    msg = f"Payment Details\n\nPlan: {duration}\nPrice: Rs {price:.2f}\n"
    kb = [
        [InlineKeyboardButton("Pay via Paytm (QR)", callback_data=f'show_qr_{order_id}')],
...
async def show_qr_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [
        [InlineKeyboardButton("Check Payment Status", callback_data='enter_utr')],
        [InlineKeyboardButton('Cancel', callback_data='cancel')]
    ]
    ...
    if qr_buffer:
        try:
            ...
            await context.bot.send_message(
                chat_id=query.message.chat_id,
                text=msg + "\n\nQR generation failed. Contact support.",
                reply_markup=InlineKeyboardMarkup(kb),
                parse_mode=ParseMode.MARKDOWN
            )
    else:
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [
        ...
        [InlineKeyboardButton('Edit Product', callback_data='edit_product_list')],
...
async def add_product_cat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"Product Added!\n\nProduct: {name}\nPID: `{pid}`\n\nNow set prices for each duration:",
        reply_markup=InlineKeyboardMarkup(kb),
        parse_mode=ParseMode.MARKDOWN
    )
    return ConversationHandler.END
...
async def save_duration_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb.append([InlineKeyboardButton('Done - Back to Admin', callback_data='admin')])
    await update.message.reply_text(
        f"Price Updated!\n\nProduct: {p['name']}\nDuration: {duration} (`{api_dur}`)\nPrice: Rs {price:.2f}\n\nSet more durations or click Done:",
        reply_markup=InlineKeyboardMarkup(kb),
        parse_mode=ParseMode.MARKDOWN
    )
    return ConversationHandler.END
...
async def view_products(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for p in products:
        ...
        dur_text = ''
        for d in durs:
            status = 'Yes' if d['is_active'] and d['admin_price'] else 'No'
...
async def set_dur_prices_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for p in products:
        pid = p['pid']
        kb.append([InlineKeyboardButton(f"Edit {p['name']}", callback_data=f'editdur_{pid}')])
...
async def edit_product_durations(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for dur in AVAILABLE_DURATIONS:
        ...
        status = 'Yes' if active and current > 0 else 'No'
...
async def save_api_key(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db.set_api_key(key)
    await update.message.reply_text(f"API Configured!\n\nKey: `{key[:15]}...`\nURL: `{FIXED_API_URL}`", parse_mode=ParseMode.MARKDOWN, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('Admin Panel', callback_data='admin')]]))
    return ConversationHandler.END
...
async def fetch_products(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if products:
        ...
        await query.edit_message_text('Failed to fetch!\n\nUse /debug command for details.', reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('Admin Panel', callback_data='admin')]]))
...
async def my_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = 'Your Orders:\n\n'
    for o in orders[:10]:
        emoji = 'Done' if o['order_status'] == 'completed' else 'Pending'
...
async def edit_product_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = []
    for p in products:
        kb.append([InlineKeyboardButton(f'Edit {p["name"]}', callback_data=f'editprod_{p["pid"]}')])
    kb.append([InlineKeyboardButton('Back', callback_data='admin')])
    await query.edit_message_text('Select Product to Edit:', reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN)
...
async def save_edit_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    p = db.get_product(pid)
    kb = [
        [InlineKeyboardButton('Edit Again', callback_data=f'editprod_{pid}')],
...
async def auto_clear_settings_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = "Auto Clear Settings\n\nCurrent Status: \" + status + \"\nClear After: \" + str(settings['hours']) + \" hours\n\nCompleted/Cancelled/Rejected orders older than \" + str(settings['hours']) + \" hours will be automatically deleted from customer history.\n\nSelect option:"
    kb = [
...
async def set_clear_hours_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    settings = db.get_auto_clear_settings()
    msg = "Set Auto Clear Hours\n\nCurrent: \" + str(settings['hours']) + \" hours\n\nEnter new value (in hours):\n\nExamples:\n- 24 = 1 day\n- 168 = 1 week\n- 720 = 30 days\n- 0 = Disable auto clear"
    await query.edit_message_text(
        msg,
        parse_mode=ParseMode.MARKDOWN,
...
async def manual_clear_now(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [[InlineKeyboardButton('Auto Clear Settings', callback_data='auto_clear_settings')]]
    msg = "Manual Clear Complete!\n\nDeleted \" + str(deleted) + \" old orders from history."
    await query.edit_message_text(
        msg,
        reply_markup=InlineKeyboardMarkup(kb),
        parse_mode=ParseMode.MARKDOWN
...
async def health_check(request):
    """GET / - UptimeRobot ke liye health check endpoint"""
    return web.Response(text="✅ KeyShop Bot v5.0 is alive!", status=200)
...
async def main_async():
    ...
    api_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(set_api_start, pattern='^set_api$', per_message=True)],
        states={SET_API_KEY: [MessageHandler(filters.TEXT & ~filters.COMMAND, save_api_key)]},
        fallbacks=[CommandHandler('cancel', start)]
    )
    add_product_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(add_product_start, pattern='^add_product$', per_message=True)],
        states={
    ...
    dur_price_conv = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(set_duration_price_start, pattern='^setdur_', per_message=True),
            CallbackQueryHandler(set_duration_price_start, pattern='^editdur_', per_message=True)
        ],
        states={ADD_PRICE: [MessageHandler(filters.TEXT & ~filters.COMMAND, save_duration_price)]},
        fallbacks=[CommandHandler('cancel', start)]
    )
    utr_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(enter_utr, pattern='^enter_utr$', per_message=True)],
        states={ENTER_UTR: [MessageHandler(filters.TEXT & ~filters.COMMAND, process_utr)]},
        fallbacks=[CallbackQueryHandler(cancel_order, pattern='^cancel$', per_message=True), CommandHandler('cancel', start)]
    )
    edit_product_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(edit_field_start, pattern='^edit_field_', per_message=True)],
        states={
    ...
    auto_clear_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(set_clear_hours_start, pattern='^set_clear_hours$', per_message=True)],
        states={SET_CLEAR_HOURS: [MessageHandler(filters.TEXT & ~filters.COMMAND, save_clear_hours)]},
        fallbacks=[CommandHandler('cancel', start)]
    )
    subadmin_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(add_subadmin_start, pattern='^add_subadmin$', per_message=True)],
    ...
    application.add_handler(CallbackQueryHandler(bot_lock_toggle, pattern='^bot_lock_toggle$', per_message=True))
    application.add_handler(CallbackQueryHandler(admin_verify_callback, pattern='^admin_verify_', per_message=True))
    application.add_handler(CallbackQueryHandler(admin_reject_callback, pattern='^admin_reject_', per_message=True))
    application.add_handler(CallbackQueryHandler(admin_cancel_callback, pattern='^admin_cancel_', per_message=True))
    application.add_handler(CallbackQueryHandler(product_details, pattern='^prod_', per_message=True))
    application.add_handler(CallbackQueryHandler(select_duration, pattern='^dur_', per_message=True))
    application.add_handler(CallbackQueryHandler(buy_product, pattern='^buy_', per_message=True))
    application.add_handler(CallbackQueryHandler(show_qr_code, pattern='^show_qr_', per_message=True))
    application.add_handler(CallbackQueryHandler(toggle_duration_callback, pattern='^toggle_', per_message=True))
    application.add_handler(CallbackQueryHandler(delete_product_callback, pattern='^delprod_', per_message=True))
    application.add_handler(CallbackQueryHandler(edit_product_durations, pattern='^editdur_', per_message=True))
    application.add_handler(CallbackQueryHandler(edit_product_list, pattern='^edit_product_list$', per_message=True))
    application.add_handler(CallbackQueryHandler(edit_product_menu, pattern='^editprod_', per_message=True))
    application.add_handler(CallbackQueryHandler(auto_clear_settings_menu, pattern='^auto_clear_settings$', per_message=True))
    application.add_handler(CallbackQueryHandler(toggle_auto_clear, pattern='^toggle_auto_clear$', per_message=True))
    application.add_handler(CallbackQueryHandler(manual_clear_now, pattern='^manual_clear$', per_message=True))
    application.add_handler(CallbackQueryHandler(subadmin_panel, pattern='^subadmin_panel$', per_message=True))
    application.add_handler(CallbackQueryHandler(subadmin_pending, pattern='^subadmin_pending$', per_message=True))
    application.add_handler(CallbackQueryHandler(manage_subadmins, pattern='^manage_subadmins$', per_message=True))
    application.add_handler(CallbackQueryHandler(remove_subadmin_list, pattern='^remove_subadmin_list$', per_message=True))
    application.add_handler(CallbackQueryHandler(remove_subadmin_callback, pattern='^removesub_', per_message=True))
    application.add_handler(CallbackQueryHandler(check_join_again, pattern='^check_join$', per_message=True))
    application.add_handler(CallbackQueryHandler(shop_now, pattern='^shop_now$', per_message=True))
    application.add_handler(CallbackQueryHandler(my_orders, pattern='^my_orders$', per_message=True))
    application.add_handler(CallbackQueryHandler(profile, pattern='^profile$', per_message=True))
    application.add_handler(CallbackQueryHandler(support, pattern='^support$', per_message=True))
    application.add_handler(CallbackQueryHandler(back_main, pattern='^back$', per_message=True))
    application.add_handler(CallbackQueryHandler(cancel_order, pattern='^cancel$', per_message=True))
    application.add_handler(CallbackQueryHandler(admin_panel, pattern='^admin$', per_message=True))
    application.add_handler(CallbackQueryHandler(fetch_products, pattern='^fetch$', per_message=True))
    application.add_handler(CallbackQueryHandler(view_products, pattern='^view_products$', per_message=True))
    application.add_handler(CallbackQueryHandler(set_dur_prices_list, pattern='^set_dur_prices$', per_message=True))
    application.add_handler(CallbackQueryHandler(pending_payments, pattern='^pending$', per_message=True))
    ...
    print('🚀 KeyShop Bot v5.0 - RENDER WEBHOOK + HEALTH CHECK FIX')
    ...
    # CRITICAL: Health check for UptimeRobot (GET /)
