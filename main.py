...
async def show_qr_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not order_id:
        try:
            ...
            await context.bot.send_message(
                ...
                text='Session expired. Start again with /start'
            )
        return
    ...
    # Try to edit existing message instead of deleting
    try:
        # First try to edit caption if it's a photo
        ...
        if qr_buffer:
            try:
                ...
                logger.error(f"QR send failed: {e2}")
                await context.bot.send_message(
                    chat_id=query.message.chat_id,
                    text=msg,
                    reply_markup=InlineKeyboardMarkup(kb),
...
async def enter_utr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Safe message editing - handles photo messages and deleted messages
    text = """Enter your UTR/Reference number:

Find it in your UPI app payment history (e.g., PhonePe, GPay, Paytm).
...
async def health_check(request):
    ...
    return web.Response(text="✅ KeyShop Bot v5.2 is alive!", status=200)
...
async def main_async():
    ...
    print('🚀 KeyShop Bot v5.2 - RENDER WEBHOOK + HEALTH CHECK FIX')
    ...
    # NOW setup job queue - AFTER application is started (FIX for weak reference error)
    job_queue = application.job_queue
    if job_queue:
        job_queue.run_repeating(auto_clear_callback, interval=3600, first=60)
        print("⏰ Auto-clear scheduled: Every 60 minutes")
    else:
        print("⚠️ WARNING: Job queue not available, auto-clear disabled")
