#!/usr/bin/env python3
"""
KeyShop Telegram Bot v5.4 - FULL FEATURES
Features:
...
async def process_utr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✅ *UTR Received!*\n\n🆔 Order: `{order_id}`\n📝 UTR: `{utr}`\n\n⏳ Payment verification pending... Admin will verify soon.",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('🏠 Main Menu', callback_data='back')]])
    )
...
async def admin_verify_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Send processing message as NEW message (don't try to edit photo message)
    ...
    # Delete processing message and send result as new message
    try:
        await processing_msg.delete()
    except:
        pass

    result_text = f"✅ *Delivered!*\n\n🆔 ID: `{order_id}`\n\n{msg}" if success else f"📄 *Result*\n\n🆔 ID: `{order_id}`\n\n{msg}"
    await query.message.reply_text(result_text, parse_mode=ParseMode.MARKDOWN, reply_markup=InlineKeyboardMarkup(final_kb))
...
async def health_check(request):
    ...
    return web.Response(text="KeyShop Bot v5.4 is alive!", status=200)
...
async def main_async():
    ...
    print('KeyShop Bot v5.4 - RENDER WEBHOOK + FULL FEATURES')
