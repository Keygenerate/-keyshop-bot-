...
try:
    ...
    print("[INIT] cloudscraper loaded - Cloudflare bypass ACTIVE")
except ImportError:
    CLOUDSCRAPER_AVAILABLE = False
    print("[INIT] cloudscraper NOT available - falling back to requests (Cloudflare may block)")
# ======================================================
...
MASTER_KEY = os.environ.get("MASTER_KEY", "a7f3e8b2c9d1f4a6b8c2d5e9f1a3b6c9")
...
class ResellerAPI:
    def __init__(self, api_key, api_url):
        ...
        if CLOUDSCRAPER_AVAILABLE:
            ...
            logger.info("[API] Cloudscraper session created - Cloudflare bypass ACTIVE")
        else:
            self.session = requests.Session()
            logger.warning("[API] Using requests.Session - Cloudflare may block this")
        # x-master-key header jo owner ne diya
        self.headers = {
    ...
    def _make_request(self, data):
        try:
            ...
            # Use cloudscraper session with x-master-key header
            ...
            if raw_text.strip().startswith('<'):
                logger.error("API returned HTML instead of JSON! Cloudflare still blocking.")
                return None, f"HTML Response (Cloudflare blocked): {raw_text[:500]}"
            try:
...
async def back_main(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # BULLETPROOF: Delete old message + send fresh main menu
    # NEVER try to edit - avoids "no text in message" error on photos
    user = update.effective_user
    is_main = db.is_main_admin(user.id)
    is_sub = db.is_sub_admin(user.id)
    text = f"👋 Welcome *{user.first_name}* to *KeyShop Bot!*\n\n🛒 Your one-stop shop for instant digital keys\n\nChoose an option below:"
    ...
    try:
        ...
        logger.info(f"Back: delete failed (already deleted?): {e}")
    ...
    try:
        ...
        logger.error(f"Back: send failed: {e2}")
...
async def cancel_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # BULLETPROOF: Delete old message + send fresh message
    # NEVER try to edit - avoids "no text in message" error on photos
    cancel_text = '❌ Order cancelled.\n\nStart again anytime!'
    cancel_kb = InlineKeyboardMarkup([[InlineKeyboardButton('🏠 Main Menu', callback_data='back')]])

    try:
        await query.delete_message()
    except Exception as e:
        logger.info(f"Cancel: delete failed (already deleted?): {e}")
    ...
    try:
        ...
        logger.error(f"Cancel: send failed: {e2}")
...
async def health_check(request):
    ...
    return web.Response(text="KeyShop Bot v5.6 - CLOUDFLARE BYPASS is alive!", status=200)
...
async def main_async():
    ...
    print('KeyShop Bot v5.6 - CLOUDFLARE BYPASS - RENDER WEBHOOK + FULL FEATURES')
