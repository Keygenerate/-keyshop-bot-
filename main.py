...
def run_web_server():
    """Run lightweight HTTP server for UptimeRobot pings"""
    try:
...
def main():
    ...
    # Start lightweight HTTP server in background thread for health checks
    ...
    if not webhook_url:
        print("\n" + "="*60)
        print("❌ FATAL ERROR: WEBHOOK_URL environment variable not set!")
        print("   Set it to: https://your-service.onrender.com/webhook")
        print("   Bot cannot start without webhook URL on Render.")
        print("="*60 + "\n")
        sys.exit(1)
    ...
    print(f"   Listen Port: {render_port}")
    print("="*60 + "\n")
