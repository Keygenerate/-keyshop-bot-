#!/usr/bin/env python3
"""
KeyShop Telegram Bot v5.0 - RENDER WEBHOOK FIX
Fixed:
- Added health check endpoint (GET /) for UptimeRobot
- Uses aiohttp web server instead of internal run_webhook()
- Proper webhook + health check on same port
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [
        ...
        [InlineKeyboardButton('🗑️ Auto Clear Settings', callback_data='auto_clear_settings')],
...
async def set_dur_prices_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for p in products:
        pid = p['pid']
        kb.append([InlineKeyboardButton(f"✏️ Edit {p['name']}", callback_data=f'editdur_{pid}')])
...
async def auto_clear_settings_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [
        ...
        [InlineKeyboardButton('🗑️ Manual Clear Now', callback_data='manual_clear')],
        [InlineKeyboardButton('🔙 Back', callback_data='admin')]
    ]
    await query.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN)
...
async def save_clear_hours(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [[InlineKeyboardButton('🗑️ Auto Clear Settings', callback_data='auto_clear_settings')]]
            ...
            if deleted > 0:
                ...
                for admin_id in ADMIN_USER_IDS:
                    try:
                        await context.bot.send_message(
                            admin_id,
                            f"🗑️ Auto Clear Report: {deleted} old orders deleted from history."
                        )
                    except:
                        pass
        except Exception as e:
